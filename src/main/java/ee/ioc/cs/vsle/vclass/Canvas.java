...
import ee.ioc.cs.vsle.editor.ISchemeContainer;
import ee.ioc.cs.vsle.editor.Menu;
import ee.ioc.cs.vsle.editor.Palette;
...
public class Canvas extends JPanel implements ISchemeContainer {
    ...
    protected Map<GObj, ClassPainter> classPainters;
    public ClassPainter currentPainter;
    protected GObj currentObj;
...
//    }
    ...
    void viewPackageComponent( String className ) {
        ...
        if( cl.getComponentType() == ComponentType.SCHEME ) {
                ClassEditor.getInstance().newSchemeTab( vPackage, getWorkDir() + className + ".syn" );
        } else {
...
//        	System.out.println("Drawing area paintComponent " + mListener.state);
    ...
    protected List<Long> m_runners = Collections.synchronizedList( new LinkedList<Long>() );
    ...
    private String genObjectName( PackageClass pClass, GObj obj ) {
        ...
        do {
        	if (pClass != null) {
        		...
        		name = obj.getName() + "_" + System.currentTimeMillis();
        	}
