        ...
        if (type == 2) {
            ...
            if (inputs.size() == 0 && outputs.size() == 1) {
                ...
                if (op.field.isPrimOrStringArray()) {
                    ...
                    assign = op.field.type + " " + " TEMP" +
                        Integer.toString(auxVarCounter) + "=" + split[1] +
                        ";\n";
                    assign += getObject(op.object) + op.name + " = TEMP" +
                        Integer.toString(auxVarCounter) + ";\n";
                    auxVarCounter++;
                    return assign;
            ...
            if (inputs.size() == 1 && outputs.size() == 1) {
                ...
                if (ip.field.isArray() && op.field.isAlias()) {
                    ...
                    for (int i = 0; i < ( (Var) outputs.get(0)).field.vars.size();
                         i++) {
                ...
                if (op.field.isArray() && ip.field.isAlias()) {
                    ...
                    assigns += op.field.type + " TEMP" +
                        Integer.toString(auxVarCounter) + " = new " +
                    ...
                    for (int i = 0; i < ip.field.vars.size(); i++) {
                        ...
                        assigns += "        " + " TEMP" +
                            Integer.toString(auxVarCounter) + "[" +
                            Integer.toString(i) + "] = " + getObject(ip.object) + s1 +
                            ";\n";
                    }
                    assigns += "        " + op + " = " + " TEMP" +
                        Integer.toString(auxVarCounter);
            ...
            for (int i = 0; i < inputs.size(); i++) {
                ...
                m = m.replaceFirst("([^a-zA-Z_]" + left2 + var.name +
                                   "[^a-zA-Z0-9_])",
                                   left + getObject(var.object) + var.name + right);
            }
            left2 = "";
            ...
            if ( ( (Var) outputs.get(0)).type.equals("int") &&
                (!getMaxType(inputs).equals("int") || method.indexOf(".") >= 0)) {
