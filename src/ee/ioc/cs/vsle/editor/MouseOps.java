...
class MouseOps extends MouseInputAdapter {
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (SwingUtilities.isRightMouseButton(e)) {
            if (State.selection.equals(state)) {
                ...
                if (relation != null) {
                    ConnectionPopupMenu popupMenu = new ConnectionPopupMenu(
                    		relation, canvas, x, y);
			...
			if (state.equals(State.addRelation)) {
				...
				if (State.isAddRelClass(state)) {
					...
					if (port != null) {
						PackageClass obj = canvas.vPackage.getClass(
								State.getClassName(state));
	...
	public void mousePressed(MouseEvent e) {
		if (state.equals(State.selection)) {
			...
			Connection con = canvas.getConnectionNearPoint(canvas.mouseX,
					canvas.mouseY);

			if (con != null) {
				draggedBreakPoint = con.breakPointContains(canvas.mouseX,
						canvas.mouseY);
				...
				if (draggedBreakPoint != null) {
					...
					obj = canvas.objects.checkInside(canvas.mouseX,
							canvas.mouseY);
				}
			} else {
				obj = canvas.objects.checkInside(canvas.mouseX,	canvas.mouseY);
			}
			...
			if (obj != null) {
				...
				cornerClicked = canvas.objects.controlRectContains(
	...
	public void mouseDragged(MouseEvent e) {
		...
		if (State.dragBreakPoint.equals(state)) {
			...
			if (RuntimeProperties.snapToGrid == 1) {
				...
				moveX = Math.round((float) (obj.getX() 
						+ Math.round((float) x / step) * step 
						- canvas.mouseX) / step) * step - obj.getX();
				moveY = Math.round((float) (obj.getY() 
						+ Math.round((float) y / step) * step 
						- canvas.mouseY) / step) * step - obj.getY();
			} else {
				...
				if (obj.isStrict()) {
					for (Port p : obj.getPorts()) {
						if (p.isStrict()) {
							...
							Port p2 = canvas.objects.getPort(
									p.getRealCenterX() + moveX,
									p.getRealCenterY() + moveY, obj);
							
							if (p2 != null && !selected.contains(p2.getObject())
									&& p.canBeConnectedTo(p2)
									&& (!p.isConnectedTo(p2)
											|| p.isStrictConnected())) {
	...
	public void mouseMoved(MouseEvent e) {
		...
		if (state.equals(State.addRelation)) {
			...
			Rectangle rect = new Rectangle(e.getX() - 10, e.getY() - 10,
					Math.round(canvas.currentObj.getRealWidth() 
							* canvas.getScale()) + 10,
					Math.round((canvas.currentObj.getRealHeight()
							* canvas.getScale()) + 10));
			
			canvas.drawingArea.scrollRectToVisible(rect);

			if (e.getX() + canvas.currentObj.getRealWidth() 
					> canvas.drawAreaSize.width) {

				canvas.drawAreaSize.width = e.getX() 
						+ canvas.currentObj.getRealWidth();

				canvas.drawingArea.setPreferredSize(canvas.drawAreaSize);
				canvas.drawingArea.revalidate();
			}

			if (e.getY() + canvas.currentObj.getRealHeight() 
					> canvas.drawAreaSize.height) {

				canvas.drawAreaSize.height = e.getY() 
					+ canvas.currentObj.getRealHeight();
	...
	/**
	 * Hilights the port under the mouse cursor when adding a relation
	 * class and the port of the relation class can be connected to the
	 * mouseover port. 
	 ...
	 */
	/*
	 * The logic for adding connections and relation classes is quite
	 * similar, is is possible to generalise and merge these methods?
	 */
	private void updateConnectionPortHilight(int x, int y) {
		...
		if (currentPort != null && currentPort != port) {
			if (canvas.currentCon == null 
					|| canvas.currentCon.beginPort != currentPort) {
				currentPort.setSelected(false);
			}
			currentPort = null;
		}
	...
	/**
	 * Hilights (selects) strict ports of the current object (the object
	 * being added or moved) that are about to be strictly connected at the
	 * current location.
	 */
	private void updateStrictPortHilight() {
		for (Port port : canvas.currentObj.getPorts()) {
			...
			Port port2 = canvas.objects.getPort(port.getRealCenterX(),
					port.getRealCenterY());

			if (port2 != null && port2.isStrict() && !port2.isStrictConnected()
					&& port.canBeConnectedTo(port2)) {
				...
				for (Port p : canvas.currentObj.getPorts()) {
					if (p.isSelected() 
							&& canvas.objects.getPort(p.getRealCenterX(),
									p.getRealCenterY()) == port2) {
						ignore = true;
						break;
					}
				...
				if (!ignore) {
					port.setSelected(true);
					canvas.currentObj.x += port2.getRealCenterX() 
							- port.getRealCenterX();
					canvas.currentObj.y += port2.getRealCenterY() 
							- port.getRealCenterY();
				}
			}
		}
	...
	public void mouseReleased(MouseEvent e) {
		...
		List<GObj> selected = canvas.objects.getSelected();
