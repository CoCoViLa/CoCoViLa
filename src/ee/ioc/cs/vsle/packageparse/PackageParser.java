...
public class PackageParser {
	...
	private final int CLASS = 1, PORT_OPEN = 2, PORT_CLOSED = 3, PACKAGE = 4,
			FIELD = 5, FIELD_KNOWN = 6;
	...
	public static final EntityResolver ENTITY_RESOLVER = new EntityResolver() {
		...
		public InputSource resolveEntity(String publicId,
			...
			if (systemId != null && systemId.endsWith("dtd")) {
				URL url = FileFuncs.getResource(
						RuntimeProperties.PACKAGE_DTD, true);
				if (url != null) {
	...
	public static VPackage createPackage( File f ) {
		return new PackageParser( f ).getPackage();
	}
	 ...
	 *            File - package file.
	 */
	private PackageParser(File file) {
		...
		if (RuntimeProperties.isLogDebugEnabled())
			db.p("Package parsing completed in "
					+ (System.currentTimeMillis() - startParsing) + "ms.\n");
	} // PackageParser

	// ===========================================================
	// SAX DocumentHandler methods
	// ===========================================================
	private class PackageHandler extends DefaultHandler implements LexicalHandler {
		...
		public void startElement(String namespaceURI, String lName,
			...
			if (element.equals(EL_OPEN)) {
				status = PORT_OPEN;
			}
			if (element.equals(EL_DEFAULT)) {
				status = FIELD;
			}
			if (element.equals(EL_KNOWN)) {
				status = FIELD_KNOWN;
			}

			if (element.equals(EL_CLOSED)) {
				status = PORT_CLOSED;
			}

			if (element.equals(EL_FIELDS)) {
				classFields = new ArrayList<ClassField>();

			}

			if (element.equals(EL_POLYGON)) {
				// initiate them to gather all X and Y positions there.
				makeInitialPolygon(attrs);

			}

			if (element.equals(EL_POINT)) {
				String x = attrs.getValue(ATR_X);
				String y = attrs.getValue(ATR_Y);
				polyXs.add(x);
				polyYs.add(y);
			...
			if (element.equals(EL_RECT)) {
				...
				String transp = attrs.getValue(ATR_TRANSPARENCY);
				...
				Rect newRect = new Rect(Integer.parseInt(x), Integer
						.parseInt(y), Integer.parseInt(width), Integer
						.parseInt(height), Integer.parseInt(color), Boolean
						.valueOf(filled).booleanValue(), str, tr, lt);
			...
			if (element.equals("oval")) {
				...
				Oval newOval = new Oval(Integer.parseInt(x), Integer
						.parseInt(y), Integer.parseInt(width), Integer
						.parseInt(height), Integer.parseInt(color), Boolean
						.valueOf(filled).booleanValue(), str, tr, lt);
			...
			if (element.equals("arc")) {
				...
				Arc newArc = new Arc(Integer.parseInt(x), Integer.parseInt(y),
						...
						Integer.parseInt(startAngle), Integer
								.parseInt(arcAngle), Integer.parseInt(color),
						Boolean.valueOf(filled).booleanValue(), str, tr, lt);

				newGraphics.addShape(newArc);
			}
			if (element.equals(EL_BOUNDS)) {
				String x = attrs.getValue(ATR_X);
				String y = attrs.getValue(ATR_Y);
				String width = attrs.getValue(ATR_WIDTH);
				String height = attrs.getValue(ATR_HEIGHT);
		...
		private Text makeText(Attributes attrs, ClassGraphics newGraphics) {
			...
			if (val.endsWith(VAL_RF)) {
				...
				fixedX = x
						- Integer.parseInt(val.substring(0, val.length() - 2));
			...
			Text newText = new Text(x, y, font, new Color(colorInt), 255, str,
					fixed);
		...
		private void makeInitialPolygon(Attributes attrs) {
			...
			polygon = new Polygon(Integer.parseInt(colour), Boolean.valueOf(
					filled).booleanValue(), str, tr, lt);
		}
		...
		private Line makeLine(Attributes attrs, ClassGraphics newGraphics) {
			...
			Line newLine = new Line(x1, y1, x2, y2, Integer.parseInt(color),
					str, tr, lt);
		...
		public void endElement(String namespaceURI, String sName, String qName)
			...
			if (qName.equals(EL_PORT)) {
				if (newPort.getOpenGraphics() == null) {
					newGraphics = new ClassGraphics();
					newGraphics.addShape(new Oval(-4, -4, 8, 8, 12632256, true,
							1.0, 255, 0));
					newGraphics.addShape(new Oval(-4, -4, 8, 8, 0, false, 1.0,
							255, 0));
				...
				if (newPort.getClosedGraphics() == null) {
					newGraphics = new ClassGraphics();
					newGraphics.addShape(new Oval(-4, -4, 8, 8, 0, true, 1.0,
							255, 0));
					newGraphics.setBounds(-4, -4, 8, 8);
		...
		public void characters(char buf[], int offset, int len)
			...
			if (element.equals(ATR_DESCRIPTION) && status == PACKAGE) {
				pack.description = s;
			}
			...
			if (element.equals(EL_ICON)) {
				newClass.icon = s;
			}
			if (element.equals(EL_PAINTER)) {
		...
		public void endCDATA() throws SAXException {
			//System.err.println("CDATASectionHandler:endCDATA " + CDATAString);
			CDATAString = "";
		...
		public void startCDATA() throws SAXException {
			//System.err.println("CDATASectionHandler:startCDATA");
		...
		public void startDTD(String name, String publicId, String systemId)
				throws SAXException {
			//System.err.println("startDTD, name: " + name + " publicId: " + publicId+ " systemId: " + systemId);
		}
