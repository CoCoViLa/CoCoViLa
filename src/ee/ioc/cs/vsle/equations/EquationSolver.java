...
public class EquationSolver {
	...
	public static void main(String[] args) throws EquationException {
		...
		for (int i = 0; i < EquationSolver.relations.size(); i++) {
			db.p(EquationSolver.relations.get(i));
		}
	}
	...
	static ExpNode upperTree() throws EquationException {
		...
		// ee.ioc.cs.editor.util.db.p(exp.inFix());
		skipBlanks();
		if (expString.peek() == '=') {
			expString.getAnyChar();
		...
		if (Character.isDigit(ch) || Character.isLetter(ch)) {
			...
			if (isFunction(val)) {
				...
				if (expString.peek() != '(') {
					...
					throw new EquationException("Error in equation '" + equation + "'. Missing left parenthesis after function name.");
				}
				...
				if (expString.peek() != ')') {
					...
					throw new EquationException("Error in equation '" + equation + "'. Missing right parenthesis after function argument.");
				}
				expString.getAnyChar(); // Read the ')'
			...
			skipBlanks();
			if (expString.peek() != ')') {
				throw new EquationException("Error in equation '" + equation + "'. Missing right parenthesis.");
			}
			expString.getAnyChar(); // Read the ")"
			return exp;
		}
		else if (ch == '\n') {
			throw new EquationException("Error in equation '" + equation + "'. End-of-line encountered in the middle of an expression.");
		}
		else if (ch == ')') {
			throw new EquationException("Error in equation '" + equation + "'. Extra right parenthesis.");
		}
		else if (ch == '+' || ch == '-' || ch == '*' || ch == '/') {
			throw new EquationException("Error in equation '" + equation + "'. Misplaced operator.");
		}
		else {
			throw new EquationException("Error in equation '" + equation + "'. Unexpected character \"" + ch + "\" encountered.");
		}

	}

	static boolean isFunction(String s) {
		if (s.equals("sin") || s.equals("cos") || s.equals("tan") || s.equals("log") || s.equals("abs")) {
			return true;
		}
		return false;
	...
	static String readWord() throws EquationException {
		...
		while (Character.isLetter(ch) || Character.isDigit(ch) || ch == '.' || ch == '_') {
			...
			if (Character.isLetter(ch) && digit) {
				throw new EquationException("Error in equation '" + equation + "'. Not a number or variable, beginning with: " + word);
			}
		}
		return word;
	}
