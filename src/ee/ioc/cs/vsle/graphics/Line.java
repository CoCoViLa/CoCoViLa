...
public class Line extends Shape implements Serializable {
    ...
    public Line(int x1, int y1, int x2, int y2, int colorInt,
    ...
    public int controlRectContains(int pointX, int pointY) {
        db.p(pointX + " " + pointY + " " + endX + " " + endY + " " + startX
                + " " + startY + " ");
        if (pointX >= startX - 2 && pointY >= startY - 2
                && pointX <= startX + 2 && pointY <= startY + 2) {
            return 1;
        }
        if (pointX >= endX - 2 && pointY >= endY - 2 && pointX <= endX + 2
                && pointY <= endY + 2) {
            return 2;
        }
        return 0;
    ...
    public boolean contains(int pointX, int pointY) {
        float distance = calcDistance(startX, startY, endX, endY, pointX,
                pointY);
        if (distance <= 3) {
            return true;
        }
    ...
    float calcDistance(int x1, int y1, int x2, int y2, int pointX, int pointY) {
        ...
        double distance = Math.sqrt((pointX - intersectX)
                * (pointX - intersectX) + (pointY - intersectY)
                * (pointY - intersectY));

        double distanceFromEnd1 = Math.sqrt((x1 - pointX) * (x1 - pointX)
                + (y1 - pointY) * (y1 - pointY));
        double distanceFromEnd2 = Math.sqrt((x2 - pointX) * (x2 - pointX)
                + (y2 - pointY) * (y2 - pointY));
    ...
    public String toFile(int boundingboxX, int boundingboxY) {
        ...
        return "<line x1=\"" + (startX - boundingboxX) + "\" y1=\""
                ...
                + "\" y2=\"" + (endY - boundingboxY) + "\" colour=\""
                + colorInt + "\" fixed=\"" + isFixed() + "\" stroke=\""
    ...
    public String toText() {
        ...
        return "LINE:" + startX + ":" + startY + ":" + endX + ":" + endY + ":"
                + colorInt + ":" + (int) stroke.getLineWidth() + ":"
